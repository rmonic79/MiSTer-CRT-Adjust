...
MiSTer-AnalogHStretch/
├── rtl/
│   └── analog_hstretch.sv      The standalone module
├── docs/
...
This guide walks through adding `analog_hstretch.sv` to an existing
...
Copy `rtl/analog_hstretch.sv` into your core's `sys/` directory (next
to `sys_top.v`) and add it to your `.qsf`:

```tcl
set_global_assignment -name SYSTEMVERILOG_FILE sys/analog_hstretch.sv
```
...
```verilog
"P1O[100:98],Analog VGA H-Stretch,0,+1,+2,+3,+4,+5,+6,+7;",
```
...
Insert the H-Stretch module **between the slot-line stream and the
...
Rebuild the core. With H-Stretch = 0 the analog output should be
...
the up-to-date `analog_hstretch.sv` and that no other code is
